...
const status = async (req, res) => {
	(0, _fiwareIota.getNodeInfo)().then(result => {
...
const setNewProvider = async (req, res) => {
	...
	res.status(HttpStatus.OK).send((0, _fiwareIota.setProvider)({ provider: provider }));
};
...
const setNewTcpProvider = async (req, res) => {
	...
	res.status(HttpStatus.OK).send((0, _fiwareIota.setTcpProvider)({ provider: provider }));
};

const createNewAccount = async (req, res) => {
	const seed = (0, _fiwareIota.generateSeed)();
	(0, _fiwareIota.createNewAddress)(seed).then(result => {
		res.status(HttpStatus.OK).send({ account: result, seed: seed });
	}).catch(err => {
		res.status(HttpStatus.INTERNAL_SERVER_ERROR).send(err);
	});
};
...
const createNewTransaction = async (req, res) => {
	...
	(0, _fiwareIota.createTransaction)(address, seed, data).then(result => {
		res.status(HttpStatus.OK).send(result);
	}).catch(err => {
		res.status(HttpStatus.INTERNAL_SERVER_ERROR).send(err);
	});
...
const fetchExistingTransaction = async (req, res) => {
	...
	(0, _fiwareIota.fetchTransaction)(address).then(result => {
		res.status(HttpStatus.OK).send(result);
	}).catch(err => {
		res.status(HttpStatus.INTERNAL_SERVER_ERROR).send(err);
	});
...
const createNewMamTransaction = async (req, res) => {
	...
	(0, _fiwareIota.createMamTransaction)(data, { mode: mode, secret: secret }).then(result => {
		res.status(HttpStatus.OK).send(result);
	}).catch(err => {
		res.status(HttpStatus.INTERNAL_SERVER_ERROR).send(err);
	});
